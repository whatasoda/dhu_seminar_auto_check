(() => {
  ...
  const dist = () => {
    ...
    const escapeRegexp = new RegExp(`(${toEscape.join('|')})`, 'g');
    ...
    const failed = lessons
      .map(
        (lesson) => [lesson, new RegExp(`;([0-9]*):${lesson.replace(escapeRegexp, '\\$1')}.*?;`)] as [string, RegExp]
      )
  ...
  const fixFormat = (lesson: string): string => lesson.replace(/([a-zⅠⅡⅢⅣ])(?=[A-Z])/g, '$1 ').replace(/-.*$/, '');
